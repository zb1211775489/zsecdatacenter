...
import com.bayee.hbase.htable.impl.juban.StRsvrfcchBHTable;
 ...
 * @Date 2021/3/11
 **/
public class StRsvrfcchBSink {

    private static final String TOPIC_NAME="st-rsvrfcch-b";
    private static final String TABLE_NAME="hb_st_rsvrfcch_b";

    public static void main(String[] args) throws Exception {

        String className = StRsvrfcchBSink.class.getName();
        ...
        props.setProperty("group.id", "st-rsvrfcch-b-group");
        ...
        mainStreaming
        .map(new MapFunction<String, StRsvrfcchBHTable>() {
            @Override
            public StRsvrfcchBHTable map(String s) throws Exception {
                return JSONObject.parseObject(s, StRsvrfcchBHTable.class);
            }
        }).map(new MapFunction<StRsvrfcchBHTable, Put>() {
            @Override
            public Put map(StRsvrfcchBHTable put) throws Exception {
                return HbaseUtils.getPut(put);
            }
        })
        .addSink(new HbaseRichSinkFunction(TABLE_NAME));

        env.execute("StRsvrfcchBSink");
    }
}

...
import com.bayee.hbase.htable.impl.juban.StRvfcchBHTable;
 ...
 * @Date 2021/3/11
 **/
public class StRvfcchBSink {

    private static final String TOPIC_NAME="st-rvfcch-b";
    private static final String TABLE_NAME="hb_st_rvfcch_b";

    public static void main(String[] args) throws Exception {

        String className = StRvfcchBSink.class.getName();
        ...
        props.setProperty("group.id", "st-rvfcch-b-group");
        ...
        mainStreaming
        .map(new MapFunction<String, StRvfcchBHTable>() {
            @Override
            public StRvfcchBHTable map(String s) throws Exception {
                return JSONObject.parseObject(s, StRvfcchBHTable.class);
            }
        }).map(new MapFunction<StRvfcchBHTable, Put>() {
            @Override
            public Put map(StRvfcchBHTable put) throws Exception {
                return HbaseUtils.getPut(put);
            }
        })
        .addSink(new HbaseRichSinkFunction(TABLE_NAME));

        env.execute("StRvfcchBSink");
    }
}

...
import com.bayee.hbase.htable.impl.shengting.ReaNatFishingBoatHTable;
import com.bayee.hbase.htable.impl.shengting.ReaNatFishingBoatRealHTable;
 ...
 * @Date 2021/3/11
 **/
public class ReaNatFishingBoatSink {

    private static final String TOPIC_NAME="rea-fishing-boat";
    private static final String TABLE_NAME="hb_rea_nat_fishing_boat";

    public static void main(String[] args) throws Exception {

        String className = ReaNatFishingBoatSink.class.getName();
        ...
        props.setProperty("group.id", "rea-nat-fishing-boat-group");
        ...
        mainStreaming
        .map(new MapFunction<String, ReaNatFishingBoatHTable>() {
            @Override
            public ReaNatFishingBoatHTable map(String s) throws Exception {
                ...
                long update_time = Long.valueOf(jsonObject.getString("update_time"));
                jsonObject.fluentPut("update_time",sdf.format(update_time));
                return JSONObject.parseObject(jsonObject.toJSONString(), ReaNatFishingBoatHTable.class);
            }
        }).map(new MapFunction<ReaNatFishingBoatHTable, Put>() {
            @Override
            public Put map(ReaNatFishingBoatHTable put) throws Exception {
                return HbaseUtils.getPut(put);
            }
        })
        .addSink(new HbaseRichSinkFunction(TABLE_NAME));

        env.execute("ReaNatFishingBoatSink");
    }
